...
void LetWidgetSay(string BipTextToDisplay)
{
    ...
    int x =
        XPLMGetDatai(XPLMFindDataRef("sim/graphics/view/window_width")) -
        (int) XPLMMeasureString(xplmFont_Proportional,
        BipTextToDisplay.c_str(), BipTextToDisplay.length()) - 10;
    int y =
        XPLMGetDatai(XPLMFindDataRef("sim/graphics/view/window_height")) - 35;
...
void WriteCSVTableToDisk(void)
{
    ...
    strftime(CSVFileName, 512,
        "Resources/plugins/Xsaitekpanels/D2B_debug_%Y-%m-%d_%H%M%S.csv",
        timeinfo);
    ...
    CSVFile << "\"Row\",\"Position\",\"Color\",\"DataRefToSet\","
        "\"DataRefType\",\"DataRefIndex\"";
...
string convert_Mac_Bip_Path(string bip_in_path)
{
    ...
    bip_in_path.erase(bip_in_path.begin() + 0,
        bip_in_path.end() - (bip_len - bip_pos));

    int bip_count = 0;
    for (size_t i = 0; i < bip_in_path.size(); i++)
        if (bip_in_path[i] == ':')
            bip_count++;
...
bool ReadConfigFile(string PlaneICAO)
{
    ...
    const char *bip1ConfigurationPath;
    const char *bip2ConfigurationPath;
    const char *bip3ConfigurationPath;
    const char *foundd2bpath,
     *foundd2bpath2,
     *foundd2bpath3;
...
#if APL && __MACH__
    std::string mac_converted_bip_path =
        convert_Mac_Bip_Path(xpsbipd2b_path_name);
    ...
    if (bipnum == bipwcscmp0) {
        bip1ConfigurationPath =
            "./Resources/plugins/Xsaitekpanels/D2B_config.txt";

        xpsbipd2b_path_name.append("D2B_config.txt");

        std::vector <
            char >parse_d2b_path_name(xpsbipd2b_path_name.size() + 1);
        std::copy(xpsbipd2b_path_name.begin(), xpsbipd2b_path_name.end(),
            parse_d2b_path_name.begin());

        std::ifstream ifile(&parse_d2b_path_name[0]);
        if (ifile) {
            XPLMDebugString("\nXsaitekpanels: Found D2B_config.txt in the "
            ...
            if (ifile) {
                XPLMDebugString("\nXsaitekpanels: Found D2B_config.txt in "
        ...
        while (getline(ReadBipFile, LineToEncrypt[0])) {
            ...
            if (LineToEncrypt[bipwcscmp0].find("[") == 0) {
                if ((LineToEncrypt[bipwcscmp0].find("[DEFAULT]") == 0)
                    || (LineToEncrypt[bipwcscmp0].find(PlaneICAO) == 0)) {
                    CorrectICAO = true;
                } else {
                    CorrectICAO = false;
                }
                if ((LineToEncrypt[bipwcscmp0].find("[DEFAULT]") != 0)
                    && (++LastMenuEntry[bipwcscmp0] < 50)) {
                    strcpy(MenuEntries[bipwcscmp0][LastMenuEntry[0]],
                        LineToEncrypt[bipwcscmp0].c_str());
            ...
            if (LineToEncrypt[bipwcscmp0].find("#RESET AUTHORITY") == 0) {
                if (++LastTableElement[bipwcscmp0] >= MAXTABLEELEMENTS) {
                    logMsg
                        ("Xdataref2BIP: Fatal Error: Too much code to handle!");
                    ReadBipFile.close();
            ...
            if (sscanf(LineToEncrypt[bipwcscmp0].c_str(),
                    "#SET BIP %c %i %c FROM ARRAY %s %i RANGE %f TO %f",
                    RowString, &BipPosition, ColorString, DataRefString,
                    &Index, &Argument, &Limit) == 7) {
                ...
                if (DataRefNumber == NULL) {
                    logMsg("Xdataref2BIP: A DataRef you want to use is not "
                        "defined!");
                    ReadBipFile.close();
                    return false;
                }
                ...
                if (!((DataRefType == xplmType_IntArray)
                        || (DataRefType == xplmType_FloatArray))) {
                    logMsg("Xdataref2BIP: A DataRef you want to use can not "
                        "be read (wrong type)!");
                    ReadBipFile.close();
                    return false;
                }
                ...
                BipTable[bipwcscmp0][LastTableElement[bipwcscmp0]].Row =
                    *RowString;
                BipTable[bipwcscmp0][LastTableElement[bipwcscmp0]].Position =
                    BipPosition;
                BipTable[bipwcscmp0][LastTableElement[bipwcscmp0]].Color =
                    *ColorString;
                BipTable[bipwcscmp0][LastTableElement
                    [bipwcscmp0]].DataRefToSet = DataRefNumber;
                BipTable[bipwcscmp0][LastTableElement[bipwcscmp0]].DataRefType
                    = DataRefType;
                BipTable[bipwcscmp0][LastTableElement
                    [bipwcscmp0]].DataRefIndex = Index;
                BipTable[bipwcscmp0][LastTableElement
                    [bipwcscmp0]].FloatValueToSet = Argument;
                BipTable[bipwcscmp0][LastTableElement[bipwcscmp0]].FloatLimit
                    = Limit;
                BipTable[bipwcscmp0][LastTableElement[bipwcscmp0]].WhatToDo =
                    'v';
                BipTable[bipwcscmp0][LastTableElement
                    [bipwcscmp0]].CSVDebugString[0] = 0;
                strcpy(BipTable[bipwcscmp0][LastTableElement
                        [bipwcscmp0]].CSVDebugString, DataRefString);
                continue;
            }
            if (sscanf(LineToEncrypt[bipwcscmp0].c_str(),
                    "#SET BIP %c %i %c FROM DATAREF %s RANGE %f TO %f",
                    RowString, &BipPosition, ColorString, DataRefString,
                    &Argument, &Limit) == 6) {
                if (++LastTableElement[bipwcscmp0] >= MAXTABLEELEMENTS) {
                    logMsg
    ...
    if (bipnum == bipwcscmp1) {
        ...
        bip2ConfigurationPath =
            "./Resources/plugins/Xsaitekpanels/D2B_config2.txt";

        xpsbipd2b_path_name2.append("D2B_config2.txt");

        std::vector <
            char >parse_d2b_path_name2(xpsbipd2b_path_name2.size() + 1);
        std::copy(xpsbipd2b_path_name2.begin(), xpsbipd2b_path_name2.end(),
            parse_d2b_path_name2.begin());

        std::ifstream ifile(&parse_d2b_path_name2[0]);
        if (ifile) {
            XPLMDebugString("\nXsaitekpanels: Found D2B_config2.txt in the "
                "current aircraft path and it is\n");
            ...
            if (ifile) {
                XPLMDebugString("\nXsaitekpanels: Found D2B_config2.txt in "
                    "the Xsaitekpanels plugin path and it is\n");
        ...
        while (getline(ReadBip2File, LineToEncrypt[bipwcscmp1])) {
            ...
            if (LineToEncrypt[bipwcscmp1].find("[") == 0) {
                ...
                if ((LineToEncrypt[bipwcscmp1].find("[DEFAULT]") == 0)
                    || (LineToEncrypt[bipwcscmp1].find(PlaneICAO) == 0)) {
                    CorrectICAO = true;
                } else {
                    CorrectICAO = false;
                }
                ...
                if ((LineToEncrypt[bipwcscmp1].find("[DEFAULT]") != 0)
                    && (++LastMenuEntry[bipwcscmp1] < 50)) {
                    strcpy(MenuEntries[bipwcscmp1][LastMenuEntry[bipwcscmp1]],
                        LineToEncrypt[bipwcscmp1].c_str());
            ...
            if (LineToEncrypt[bipwcscmp1].find("#RESET AUTHORITY") == 0) {
                ...
                BipTable[bipwcscmp1][LastTableElement[bipwcscmp1]].WhatToDo =
                    'T';
                continue;
            }

            if (sscanf(LineToEncrypt[bipwcscmp1].c_str(),
                    "#SET BIP %c %i %c FROM ARRAY %s %i RANGE %f TO %f",
                    RowString, &BipPosition, ColorString, DataRefString,
                    &Index, &Argument, &Limit) == 7) {
                if (++LastTableElement[bipwcscmp1] >= MAXTABLEELEMENTS) {
                ...
                BipTable[bipwcscmp1][LastTableElement[bipwcscmp1]].Row =
                    *RowString;
                BipTable[bipwcscmp1][LastTableElement[bipwcscmp1]].Position =
                    BipPosition;
                BipTable[bipwcscmp1][LastTableElement[bipwcscmp1]].Color =
                    *ColorString;
                BipTable[bipwcscmp1][LastTableElement
                    [bipwcscmp1]].DataRefToSet = DataRefNumber;
                BipTable[bipwcscmp1][LastTableElement[bipwcscmp1]].DataRefType
                    = DataRefType;
                BipTable[bipwcscmp1][LastTableElement
                    [bipwcscmp1]].DataRefIndex = Index;
                BipTable[bipwcscmp1][LastTableElement
                    [bipwcscmp1]].FloatValueToSet = Argument;
                BipTable[bipwcscmp1][LastTableElement[bipwcscmp1]].FloatLimit
                    = Limit;
                BipTable[bipwcscmp1][LastTableElement[bipwcscmp1]].WhatToDo =
                    'v';
                BipTable[bipwcscmp1][LastTableElement
                    [bipwcscmp1]].CSVDebugString[0] = 0;
                strcpy(BipTable[bipwcscmp1][LastTableElement
                        [bipwcscmp1]].CSVDebugString, DataRefString);
                continue;
            }
            if (sscanf(LineToEncrypt[bipwcscmp1].c_str(),
                    "#SET BIP %c %i %c FROM DATAREF %s RANGE %f TO %f",
    ...
    if (bipnum == bipwcscmp2) {
        bip3ConfigurationPath =
            "./Resources/plugins/Xsaitekpanels/D2B_config3.txt";
        xpsbipd2b_path_name3.append("D2B_config3.txt");

        std::vector <
            char >parse_d2b_path_name3(xpsbipd2b_path_name3.size() + 1);
        std::copy(xpsbipd2b_path_name3.begin(), xpsbipd2b_path_name3.end(),
            parse_d2b_path_name3.begin());

        std::ifstream ifile(&parse_d2b_path_name3[0]);
        if (ifile) {
            XPLMDebugString("\nXsaitekpanels: Found D2B_config3.txt in the "
                "current aircraft path and it is\n");
            ...
            if (ifile) {
                XPLMDebugString("\nXsaitekpanels: Found D2B_config3.txt "
                    "in the Xsaitekpanels plugin path and it is\n");
        ...
        while (getline(ReadBip3File, LineToEncrypt[bipwcscmp2])) {
            ...
            if (LineToEncrypt[bipwcscmp2].find("[") == 0) {
                ...
                if ((LineToEncrypt[bipwcscmp2].find("[DEFAULT]") == 0)
                    || (LineToEncrypt[bipwcscmp2].find(PlaneICAO) == 0)) {
                    CorrectICAO = true;
                } else {
                    CorrectICAO = false;
                }
                ...
                if ((LineToEncrypt[bipwcscmp2].find("[DEFAULT]") != 0)
                    && (++LastMenuEntry[bipwcscmp2] < 50)) {
                    strcpy(MenuEntries[bipwcscmp2][LastMenuEntry[bipwcscmp2]],
                        LineToEncrypt[bipwcscmp2].c_str());
            ...
            if (LineToEncrypt[bipwcscmp2].find("#RESET AUTHORITY") == 0) {
                ...
                BipTable[bipwcscmp2][LastTableElement[bipwcscmp2]].WhatToDo =
                    'T';
                continue;
            }

            if (sscanf(LineToEncrypt[bipwcscmp2].c_str(),
                    "#SET BIP %c %i %c FROM ARRAY %s %i RANGE %f TO %f",
                    RowString, &BipPosition, ColorString, DataRefString,
                    &Index, &Argument, &Limit) == 7) {
                if (++LastTableElement[bipwcscmp2] >= MAXTABLEELEMENTS) {
                ...
                DataRefNumber = XPLMFindDataRef(DataRefString);
                if (DataRefNumber == NULL) {
                    logMsg("Xdataref3BIP: A DataRef you want to use is not "
                        "defined!");
                    ReadBip3File.close();
                    return false;
                }
                ...
                if (!((DataRefType == xplmType_IntArray)
                        || (DataRefType == xplmType_FloatArray))) {
                    logMsg("Xdataref3BIP: A DataRef you want to use can not "
                        "be read (wrong type)!");
                    ReadBip3File.close();
                    return false;
                }
                ...
                BipTable[bipwcscmp2][LastTableElement[bipwcscmp2]].Row =
                    *RowString;
                BipTable[bipwcscmp2][LastTableElement[bipwcscmp2]].Position =
                    BipPosition;
                BipTable[bipwcscmp2][LastTableElement[bipwcscmp2]].Color =
                    *ColorString;
                BipTable[bipwcscmp2][LastTableElement
                    [bipwcscmp2]].DataRefToSet = DataRefNumber;
                BipTable[bipwcscmp2][LastTableElement[bipwcscmp2]].DataRefType
                    = DataRefType;
                BipTable[bipwcscmp2][LastTableElement
                    [bipwcscmp2]].DataRefIndex = Index;
                BipTable[bipwcscmp2][LastTableElement
                    [bipwcscmp2]].FloatValueToSet = Argument;
                BipTable[bipwcscmp2][LastTableElement[bipwcscmp2]].FloatLimit
                    = Limit;
                BipTable[bipwcscmp2][LastTableElement[bipwcscmp2]].WhatToDo =
                    'v';
                BipTable[bipwcscmp2][LastTableElement
                    [bipwcscmp2]].CSVDebugString[0] = 0;
                strcpy(BipTable[bipwcscmp2][LastTableElement
                        [bipwcscmp2]].CSVDebugString, DataRefString);
                continue;
            }
            if (sscanf(LineToEncrypt[bipwcscmp2].c_str(),
                    "#SET BIP %c %i %c FROM DATAREF %s RANGE %f TO %f",
                ...
                if (++LastTableElement[bipwcscmp2] >= MAXTABLEELEMENTS) {
                    logMsg(
                        "Xdataref3BIP: Fatal Error: Too much code to handle!");
                    ReadBip3File.close();
                    LastTableElement[bipwcscmp2] = MAXTABLEELEMENTS - 1;
                    return false;
                }
...
void process_bip_what_to_do_v()
{
    ...
    for (i[bipnum] = 0; i[bipnum] <= LastTableElement[bipnum]; i[bipnum]++) {
        ...
        if (BipTable[bipnum][i[bipnum]].WhatToDo == 'v') {
            if (BipTable[bipnum][i[bipnum]].DataRefType == xplmType_Float)
                LookAtThisValue =
                    XPLMGetDataf(BipTable[bipnum][i[bipnum]].DataRefToSet);
            if (BipTable[bipnum][i[bipnum]].DataRefType == xplmType_Int)
                LookAtThisValue =
                    (float)
                    XPLMGetDatai(BipTable[bipnum][i[bipnum]].DataRefToSet);
            if (BipTable[bipnum][i[bipnum]].DataRefType ==
                xplmType_FloatArray)
                XPLMGetDatavf(BipTable[bipnum][i[bipnum]].DataRefToSet,
                    &LookAtThisValue,
                    BipTable[bipnum][i[bipnum]].DataRefIndex, 1);
            if (BipTable[bipnum][i[bipnum]].DataRefType == xplmType_IntArray) {
                XPLMGetDatavi(BipTable[bipnum][i[bipnum]].DataRefToSet,
                    &LookAtThisIntValue,
                    BipTable[bipnum][i[bipnum]].DataRefIndex, 1);
                LookAtThisValue = (float) LookAtThisIntValue;
            }
            if ((LookAtThisValue >=
                    BipTable[bipnum][i[bipnum]].FloatValueToSet)
                && (LookAtThisValue <=
                    BipTable[bipnum][i[bipnum]].FloatLimit)) {

                if (BipTable[bipnum][i[bipnum]].Row == 'A') {
                    if (BipTable[bipnum][i[bipnum]].Color == 'G') {
                        // Make BipTable[i].Position position Top Row Green
                        bipwbuf[bipnum][1] |=
                            (1 << (BipTable[bipnum][i[bipnum]].Position)),
                            bipwbuf[bipnum][4] &=
                            ~(1 << (BipTable[bipnum][i[bipnum]].Position));
                    }
                    ...
                    for (i[bipnum] = 0; i[bipnum] <= LastTableElement[bipnum];
                        i[bipnum]++) {
                        ...
                        if (BipTable[bipnum][i[bipnum]].WhatToDo == 'v') {
                            ...
                            if (BipTable[bipnum][i[bipnum]].DataRefType ==
                                xplmType_Float)
                                LookAtThisValue =
                                    XPLMGetDataf(BipTable[bipnum][i
                                        [bipnum]].DataRefToSet);
                            if (BipTable[bipnum][i[bipnum]].DataRefType ==
                                xplmType_Int)
                                LookAtThisValue = (float)
                                    XPLMGetDatai(BipTable[bipnum][i
                                        [bipnum]].DataRefToSet);
                            if (BipTable[bipnum][i[bipnum]].DataRefType ==
                                xplmType_FloatArray)
                                XPLMGetDatavf(BipTable[bipnum][i
                                        [bipnum]].DataRefToSet,
                                    &LookAtThisValue,
                                    BipTable[bipnum][i[bipnum]].DataRefIndex,
                                    1);
                            if (BipTable[bipnum][i[bipnum]].DataRefType ==
                                xplmType_IntArray) {
                                XPLMGetDatavi(BipTable[bipnum][i
                                        [bipnum]].DataRefToSet,
                                    &LookAtThisIntValue,
                                    BipTable[bipnum][i[bipnum]].DataRefIndex,
                                    1);
                            ...
                            if ((LookAtThisValue >=
                                    BipTable[bipnum][i
                                        [bipnum]].FloatValueToSet)
                                && (LookAtThisValue <=
                                    BipTable[bipnum][i[bipnum]].FloatLimit)) {

                                if (BipTable[bipnum][i[bipnum]].Row == 'A') {
                                    if (BipTable[bipnum][i[bipnum]].Color ==
                                        'G') {
                                        // Make BipTable[i].Position position Top Row Green
                                        bipwbuf[bipnum][1] |=
                                            (1 <<
...
void process_bip_panel()
{
    ...
    if (biploop[bipnum] == 2) {
        if (memcmp(bipwbuf[bipnum], lastbipwbuf[bipnum], 10) != 0) {
            if (BatPwrIsOn()) {
                res = hid_send_feature_report(biphandle[bipnum],
                    bipwbuf[bipnum], 10);
        ...
        if (!BatPwrIsOn()) {
            ...
            bipwbuf[bipnum][5] = 0;
            bipwbuf[bipnum][6] = 0;
            res = hid_send_feature_report(biphandle[bipnum], bipwbuf[bipnum],
                10);
            memcpy(lastbipwbuf[bipnum], bipwbuf[bipnum], 10);
        }
    }
