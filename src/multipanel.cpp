...
static int altdbncinc = 0,
    altdbncdec = 0,
    vsdbncinc = 0,
    vsdbncdec = 0;
static int iasdbncinc = 0,
    iasdbncdec = 0,
    hdgdbncinc = 0,
    hdgdbncdec = 0;
static int crsdbncinc = 0,
    crsdbncdec = 0;

static float upapaltf,
  upapvsf,
  upapasf,
  upaphdgf,
  upapcrsf,
...
static int multiaactv,
  multiadig1,
  multiarem1,
  multiadig2,
  multiarem2;
static int multiadig3,
  multiarem3,
  multiadig4,
  multiarem4,
  multiadig5;
static int multibstby,
  multibdig1,
  multibdig2,
  multibrem2;
static int multibdig3,
  multibrem3,
  multibdig4,
  multibrem4,
  multibdig5;
static int btnleds = 0,
    lastbtnleds = 0,
    multiseldis = 5;

static int ALT_SWITCH = 7,
    VS_SWITCH = 6;
static int IAS_SWITCH = 5,
    HDG_SWITCH = 4;
static int CRS_SWITCH = 3,
    AUTO_THROTTLE_SWITCH = 8;
static int AP_MASTER_BUTTON = 0,
    HDG_BUTTON = 15;
static int NAV_BUTTON = 14,
    IAS_BUTTON = 13;
static int ALT_BUTTON = 12,
    VS_BUTTON = 11;
static int APR_BUTTON = 10,
    REV_BUTTON = 9;
static int FLAPS_UP_SWITCH = 23,
    FLAPS_DN_SWITCH = 22;
static int TRIM_WHEEL_UP = 20,
    TRIM_WHEEL_DN = 21;
static int ADJUSTMENT_UP = 2,
    ADJUSTMENT_DN = 1;

static int Last_Adjustment_Up,
  Last_Adjustment_Dn;
...
void process_multi_menu()
{
    ...
    XPLMAppendMenuItem(MultiMenuId, "Multi Panel Widget",
        (void *) "MULTI_WIDGET", 1);
}
...
void process_multi_display()
{
    ...
    default:
        ...
        multiadig1 = 15, multiadig2 = 15, multiadig3 = 15, multiadig4 =
            15, multiadig5 = 15;
        multibdig1 = 15, multibdig2 = 15, multibdig3 = 15, multibdig4 =
            15, multibdig5 = 15;
        btnleds = 0;
        break;
    }
// ****** Make Message One Digit at A Time and Turn on Button LEDS  *******
    char multiadigit1 = multiadig1,
        multiadigit2 = multiadig2,
        multiadigit3 = multiadig3;
    char multiadigit4 = multiadig4,
        multiadigit5 = multiadig5;
    char multibdigit1 = multibdig1,
        multibdigit2 = multibdig2,
        multibdigit3 = multibdig3;
    char multibdigit4 = multibdig4,
        multibdigit5 = multibdig5;
    char cdigit1 = btnleds;

// ****** Load Array with Message of Digits and Button LEDS *************
    multiwbuf[0] = 0;
    multiwbuf[1] = multiadigit1, multiwbuf[2] = multiadigit2, multiwbuf[3] =
        multiadigit3;
    multiwbuf[4] = multiadigit4, multiwbuf[5] = multiadigit5;
    multiwbuf[6] = multibdigit1, multiwbuf[7] = multibdigit2, multiwbuf[8] =
        multibdigit3;
    multiwbuf[9] = multibdigit4, multiwbuf[10] = multibdigit5, multiwbuf[11] =
        cdigit1;
...
void process_multi_panel_datareference_values()
{
    ...
    if (Last_Adjustment_Up != 0 && adjustment_up == 0) {
        XPLMSetDatai(MultiKnobIncTicksOwnedDataRef,
            XPLMGetDatai(MultiKnobIncTicksOwnedDataRef) + 1);
    }
    int adjustment_dn = XPLMGetDatai(MultiKnobDecOwnedDataRef);
    if (Last_Adjustment_Dn != 0 && adjustment_dn == 0) {
        XPLMSetDatai(MultiKnobDecTicksOwnedDataRef,
            XPLMGetDatai(MultiKnobDecTicksOwnedDataRef) + 1);
    }
}
...
void process_alt_switch()
{
    if (testbit(multibuf, ALT_SWITCH)) {
        ...
        if ((Last_Adjustment_Up == 1)
            && (testbit(multibuf, ADJUSTMENT_UP) == 0)) {
            altdbncinc++;
        ...
        if ((Last_Adjustment_Dn == 1)
            && (testbit(multibuf, ADJUSTMENT_DN) == 0)) {
            altdbncdec++;
            ...
            if (iasdbncinc > multispeed) {
                ...
                if (xpanelsfnbutton == 1) {
                    if (iasswitchremap == 1) {
                        ...
                        while (n > 0) {
                            if (iasismachremap == 1) {
                                if (XPLMGetDatai(IasIsmachRemapableData) ==
                                    iasismachvalue) {
                                    apmasf = apmasf + .01;
                                } else {
                                    apas = apas + 1;
        ...
        case 1:
            ...
            if (lightdatareferencetype == 1) {
                ...
                case 0:
                    ...
                    } else if (XPLMGetDataf(NavLightVorlocFlashRemapableData)
                        < .50) {
                ...
                case 0:
                    navlightflashvorlocdata =
                        XPLMGetDatai(NavLightVorlocFlashRemapableData);
                ...
                navlightvorlocdata =
                    XPLMGetDatai(NavLightVorlocRemapableData);
                switch (navlightvorlocdata) {
                case 2:
        ...
        case 2:
            if (lightdatareferencetype == 1) {
                ...
                case 0:
                    ...
                    } else if (XPLMGetDataf(NavLightLnavFlashRemapableData) <
                        .50) {
                ...
                case 0:
                    navlightflashlnavdata =
                        XPLMGetDatai(NavLightLnavFlashRemapableData);
...
void process_multi_panel()
{
    ...
    do {
        ...
        if (multires > 0) {
            ...
            hid_send_feature_report(multihandle, multiwbuf,
                sizeof(multiwbuf));
    ...
    if ((lastmultiseldis != multiseldis) || (lastbtnleds != btnleds)
        || (multinowrite > 50)) {
        mulres =
            hid_send_feature_report(multihandle, multiwbuf,
            sizeof(multiwbuf));
